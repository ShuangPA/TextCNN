...
class TextCNN(object):
  def __init__(self, sequenceLength, numClasses, vocabSize,
    ...
    #a = np.zeros((vocabSize, embeddingSize))
    #self.w = tf.get_variable(name="WW", shape=[vocabSize, embeddingSize], initializer=tf.constant_initializer(a), trainable=True)
    ...
    #print(self.embeddedCharsExpanded.get_shape())
    ...
    self.hDrop = tf.nn.dropout(self.hPoolFlat, self.dropout_keep_prob)
    ...
    self.w_scores = tf.nn.softmax(self.scores, 1, name='w_scores')
